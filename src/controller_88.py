    ...
    def calculate_potential_field(self, idx_robot, idx_workstand):
        ...
        for idx_other in range(4):
            if not idx_other == idx_robot:
                ...
                v_robot = math.sqrt(self._robots.get_status(feature_line_velo_x_r, idx_robot) ** 2 + self._robots.get_status(feature_line_velo_y_r, idx_robot) ** 2)
                v_other = math.sqrt(self._robots.get_status(feature_line_velo_x_r, idx_other) ** 2 + self._robots.get_status(feature_line_velo_y_r, idx_other) ** 2)
                m_robot = self._robots.get_status(feature_materials_r, idx_robot)
                m_other = self._robots.get_status(feature_materials_r, idx_other)
                ...
                # # 如果机器人之间的距离小于一定半径范围，则计算斥力
                if distance_robot < RADIUS and m_robot <= m_other and idx_robot < idx_other: # and (ang_robot < math.pi * 0.3 or ang_other < math.pi * 0.3):
                    repulsive_force = 0.5 * idx_robot * ETA * \
                        ((1.0 / distance_robot) - (1.0 / RADIUS)) ** 2
                    repulsive_field[0] -= repulsive_force * dx_robot
                    repulsive_field[1] -= repulsive_force * dy_robot
