...
class Map:
    ...
    def set_product_pro(self, idx_workstand, value):
        # 预购接口, 如果是123特殊处理
        logger.info(str(idx_workstand)+" "+str(value))
    ...
    def control(self):
            ...
            for sell_workstand_idx in range(len(self._workstands)):
                ...
                for robot_idx in range(4):
                    ...
                    ratio = (price_sell * rate(price_sell) - price_buy) \
                            / (dis_product_sell_workstand * 4 + \
                               (max(dis_robot_product_workstand * 4, product_time)))
        ...
        while idx_robot < 4:
            ...
            if robot_status == 0:
                ...
                # 判断距离是否够近
                if self.get_dis_robot2workstand(idx_robot, self._robots.get_status(feature_target_r, idx_robot)) < 1:
                    # 减速
                    self.move2loc(idx_robot, 0.3)
                # 判定是否进入交互范围
                ...
                # 判断距离是否够近
                if self.get_dis_robot2workstand(idx_robot, self._robots.get_status(feature_target_r, idx_robot)) < 1:
                    self.move2loc(idx_robot, 0.3)  # 减速
                ...
                if self._robots.get_status(feature_workstand_id_r, idx_robot) == self._robots.get_status(
                    ...
                    idx_robot += 1
                    continue
                self.move2loc(idx_robot,  999)  # 加速开车
                ...
                logger.info(str(sell_workstand_idx)+",mate"\
                            +str(self._workstands.get_workstand_status(sell_workstand_idx)[2])+",protype"+str(material_type))
                can_put_material = (int(materials >>  material_type) & 1) == 0
                logger.info("can put material:" + str(can_put_material))
                if is_nearby_workstand and can_put_material:  # 能否放原材料
                    ...
                    self._workstands.set_material_pro(sell_workstand_idx, clrbit(int(materials),int(material_type)))  # 取消预购
